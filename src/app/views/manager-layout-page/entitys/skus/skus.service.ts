...
import { SkusHttpService } from '@backend';
import { PaginatorState } from 'primeng/paginator';

@Injectable({ providedIn: 'root' })
export class SkusService {
  readonly items$ = new BehaviorSubject<Sku[]>([]);
  readonly total$ = new BehaviorSubject<number>(0);
  readonly loading$ = new BehaviorSubject<boolean>(false);

  constructor(private http: SkusHttpService) {}

  load(paginator: PaginatorState, productIdFilter = ''): void {
    this.loading$.next(true);
    this.http.getAll$({
      skip: paginator.first ?? 0, take: paginator.rows ?? 10,
      sortBy: null, sortDirection: 0,
      id: { matchMode: 'Equals', value: '' },
      productId: { matchMode: 'Equals', value: productIdFilter },
      price: { matchMode: 'Equals', value: '' },
      amount: { matchMode: 'Equals', value: '' },
      discount: { matchMode: 'Equals', value: '' },
    }).pipe(
      timeout(15000),
      catchError(() => of(null)),
  ...
  create$(item: Partial<Sku>): Observable<Sku> {
  ...
  update$(item: Sku): Observable<Sku> {
